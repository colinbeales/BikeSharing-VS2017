...
namespace Trainer
{
	public class Trainer
	{
		...
		public (Intensity, int) MostFrequentIntensity()
		{
			...
			foreach (var dictionaryValues in IntensityDictionary)
			{
				if (dictionaryValues.Value > highestCount.Item2)
                { 
					highestCount = (dictionaryValues.Key, dictionaryValues.Value);
				}
			}
			return highestCount;
		}
        ...
        public async Task<bool> SaveWorkouts()
		{
			using (StreamWriter writer = File.CreateText("MyWorkouts.csv"))
			{
				await writer.WriteLineAsync("Miles, Duration");
				
				foreach (var workout in _workOuts)
				{
					await writer.WriteLineAsync($"{workout.Miles},{workout.Duration}");
				}
			}
			return true;
		}
